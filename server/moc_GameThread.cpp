/****************************************************************************
** Meta object code from reading C++ file 'GameThread.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.0.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "game/GameThread.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'GameThread.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.0.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_GameThread_t {
    QByteArrayData data[17];
    char stringdata[140];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_GameThread_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_GameThread_t qt_meta_stringdata_GameThread = {
    {
QT_MOC_LITERAL(0, 0, 10),
QT_MOC_LITERAL(1, 11, 14),
QT_MOC_LITERAL(2, 26, 0),
QT_MOC_LITERAL(3, 27, 3),
QT_MOC_LITERAL(4, 31, 17),
QT_MOC_LITERAL(5, 49, 6),
QT_MOC_LITERAL(6, 56, 6),
QT_MOC_LITERAL(7, 63, 10),
QT_MOC_LITERAL(8, 74, 11),
QT_MOC_LITERAL(9, 86, 7),
QT_MOC_LITERAL(10, 94, 4),
QT_MOC_LITERAL(11, 99, 5),
QT_MOC_LITERAL(12, 105, 4),
QT_MOC_LITERAL(13, 110, 4),
QT_MOC_LITERAL(14, 115, 11),
QT_MOC_LITERAL(15, 127, 2),
QT_MOC_LITERAL(16, 130, 8)
    },
    "GameThread\0writeToConsole\0\0str\0"
    "updatePacketReady\0Packet\0packet\0"
    "runCommand\0CommandType\0command\0args\0"
    "start\0stop\0quit\0threadSleep\0ms\0mainLoop\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_GameThread[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   54,    2, 0x05,
       4,    1,   57,    2, 0x05,

 // slots: name, argc, parameters, tag, flags
       7,    2,   60,    2, 0x0a,
      11,    0,   65,    2, 0x0a,
      12,    0,   66,    2, 0x0a,
      13,    0,   67,    2, 0x0a,
      14,    1,   68,    2, 0x0a,
      16,    0,   71,    2, 0x08,

 // signals: parameters
    QMetaType::Void, QMetaType::QString,    3,
    QMetaType::Void, 0x80000000 | 5,    6,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 8, QMetaType::QStringList,    9,   10,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::UInt,   15,
    QMetaType::Void,

       0        // eod
};

void GameThread::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        GameThread *_t = static_cast<GameThread *>(_o);
        switch (_id) {
        case 0: _t->writeToConsole((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 1: _t->updatePacketReady((*reinterpret_cast< Packet(*)>(_a[1]))); break;
        case 2: _t->runCommand((*reinterpret_cast< CommandType(*)>(_a[1])),(*reinterpret_cast< QStringList(*)>(_a[2]))); break;
        case 3: _t->start(); break;
        case 4: _t->stop(); break;
        case 5: _t->quit(); break;
        case 6: _t->threadSleep((*reinterpret_cast< uint(*)>(_a[1]))); break;
        case 7: _t->mainLoop(); break;
        default: ;
        }
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        switch (_id) {
        default: *reinterpret_cast<int*>(_a[0]) = -1; break;
        case 1:
            switch (*reinterpret_cast<int*>(_a[1])) {
            default: *reinterpret_cast<int*>(_a[0]) = -1; break;
            case 0:
                *reinterpret_cast<int*>(_a[0]) = qRegisterMetaType< Packet >(); break;
            }
            break;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (GameThread::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&GameThread::writeToConsole)) {
                *result = 0;
            }
        }
        {
            typedef void (GameThread::*_t)(Packet );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&GameThread::updatePacketReady)) {
                *result = 1;
            }
        }
    }
}

const QMetaObject GameThread::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_GameThread.data,
      qt_meta_data_GameThread,  qt_static_metacall, 0, 0}
};


const QMetaObject *GameThread::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *GameThread::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_GameThread.stringdata))
        return static_cast<void*>(const_cast< GameThread*>(this));
    return QObject::qt_metacast(_clname);
}

int GameThread::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void GameThread::writeToConsole(const QString & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void GameThread::updatePacketReady(Packet _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
